...
$script = <<-'SCRIPT'
apt-get update -y
apt-get install -y afl afl++ afl-clang
...
sudo -u vagrant -D ~vagrant wget https://github.com/cs-uob/COMSM0049/blob/master/docs/labs/heartbeat_normal.bin -O AFL/temp-in/heartbeat_normal.bin
sudo -u vagrant -D ~vagrant wget https://github.com/cs-uob/COMSM0049/blob/master/docs/labs/heartbleed.c -O heartbleed.c
SCRIPT
...
1. If you're not using the VM, Install [AFL] (https://github.com/google/AFL). you can directly download the zip, or use your package manager (`apt install afl++` on Debian)
2. If you're building from source, compile it.  Read the `README.md` (but I'd just use a package manager).
