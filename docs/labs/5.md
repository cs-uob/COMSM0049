...
1. Copy the above seed input, and source into the `temp-in` directory (done for you on VM)
...
13. Now, check the crash field in TUI. In the crash field, you will see number of crashes. Let the fuzzer run for 15-20 min. Number of unique crashes should increase (max will be ~5). One of them should be your heartbleed bug (hurray :D ).
...
16. One fo such input should give you the following output:
