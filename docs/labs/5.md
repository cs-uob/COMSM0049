...
I have implemented a simple version of heartbleed vulnerability. Check [heartbleed.c](heartbleed.c). Read the comments in the code to find out what is the bug and how it can be triggered. Our goal for the lab is to find the bug via fuzzing. You will need a seed input to start fuzzing. use [heartbeat_normal.bin](heartbeat_normal.bin) as seed input. You can use your Ubuntu 64-bit OS.
Steps:
